...
var FACTORY_ADDRESS = '0x7748893602a1421B9Dc4d240e5185FB1634bcFFE';
...
var NonfungiblePositionManager = /*#__PURE__*/function () {
  ...
  NonfungiblePositionManager.addCallParameters = function addCallParameters(position, options) {
    ...
    var _position$mintAmounts = position.mintAmounts,
        ...
        amount1Desired = _position$mintAmounts.amount1; // 调整滑点
    ...
    var deadline = toHex(options.deadline); // 输出position和options的相关信息
    ...
    console.log("Deadline:", deadline); // 如果需要创建池
    ...
    if (isMint(options)) {
      var recipient = sdkCore.validateAndParseAddress(options.recipient); // 打印 mint 参数信息
    ...
    if (options.useNative) {
      ...
      console.log("Use Native Token. Wrapped Value:", wrappedValue.toString()); // 只有在真正发送 ETH 时才需要退款
